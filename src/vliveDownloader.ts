...
export class VliveDownloader {
  ...
  public async getVideoKey() {
    const data = await needle(
      "get",
      `https://www.vlive.tv/globalv-web/vam-web/video/v1.0/vod/${this.videoSeq}/inkey?appId=8c6cc7b45d2568fb668be6e05b6e5a3b&platformType=PC&gcc=KR&locale=en_US`,
      this.getPostOption()
    ).then((res) => res.body);
  ...
  public async getVideoMetaData() {
    const data = await needle(
      "get",
      `https://apis.naver.com/rmcnmv/rmcnmv/vod/play/v2.0/${this.officialVideo.vodId}?key=${this.videoKey}&videoId=${this.officialVideo.vodId}&cc=US`,
      this.getPostOption()
    ).then((res) => res.body);
